...
/**
 * <p>
 * MLog是一个输出到Logcat和文件的日志工具
 * </p>
 *
 * <li>输出的文本会加入进程id[(P:666)]、线程id信息[(T:0)代表主线程]</li>
 * <li>error信息传入Throwable对象，可以输出完整错误栈</li>
 * <li>LogOptions中可以设置输出的日志级别</li>
 * <li>须调用initialize()初始化后再使用</li>
 * <li>建议使用format格式的字符串参数输出日志，减少字符串的手工拼接</li>
 * <li></li>
 *
 * <p/>
 */
...
public class MLog {
  public final static String EXT = ".lg";
  ...
  public static class LogOptions {
     ...
     */
    public String logFileName = "logs" + EXT;
  ...
  private static void writeToLog(final String logText) {
    ...
    if (externalStorageExist()) {
      try {
        ...
        do {
          if (!LogToES.ready(fileName)) {
            break;
          }
          if (LogToES.isSizeLimited()) {
            break;
          }
          LogToES.writeLogToFile(logText, false, timeMillis);
        } while (false);
      } catch (Throwable e) {
  ...
  /*
   * private static String msgForTextLog(int logLevel, Object obj, String
   * filename, int line,
   * String msg) {
  ...
  private static void outputVerbose(final Object obj, final String format, final boolean outToDDMS,
      final boolean outToFile, final Object... args) {
    final Runnable command = new Runnable() {
      @Override
      public void run() {
        try {
  ...
  private static void outputVerbose(final Object obj, final int line,
      final String filename, final String format, final String thread, final boolean outToDDMS, final boolean outToFile,
      final Object... args) {
    final Runnable command = new Runnable() {
      @Override
      public void run() {
